...
def FI(sz=12):
    return (
        "Arial-Italic"
        if "Arial-Italic" in pdfmetrics.getRegisteredFontNames()
...
class D:
    ...
    def hdr(self, title, sec=""):
        ...
        cv.rect(0, H - TH, W, TH, fill=1, stroke=0)
        cv.setFillColor(ACT)
        cv.rect(0, H - TH - 3, W, 3, fill=1, stroke=0)
    ...
    def tx(self, x, y, s, bold=False, italic=False, sz=12, cl=None, a="l"):
        ...
        cv.setFont(font, sz)
        draw = {"c": cv.drawCentredString, "r": cv.drawRightString}.get(
            a, cv.drawString
        )
...
def build(path):
    ...
    d.tx(
        W // 2,
        308,
        "A Lecture on Deep Reinforcement Learning",
        italic=True,
        sz=16,
        cl=MGRY,
        a="c",
    ...
    d.tx(
        W // 2,
        70,
        "Predator–Prey Archetype Gridworld  ·  2026",
        sz=10,
        cl=HexColor("#455A64"),
        a="c",
    )
    ...
    d.page()
    d.hdr("The Limits of Tabular RL", S1)
    y = CT
    ...
    d.page()
    d.hdr("The Curse of Dimensionality", S1)
    y = CT
    d.tx(ML, y, "State space grows exponentially with problem complexity.", sz=13)
    y -= 30

    items = [
        ("Simple Grid", "4 × 4 = 16 states", GREEN, LGRN),
    ...
    d.tx(
        ML + 140,
        y - 12,
        "We need to generalise across states — not memorise each one.",
        sz=12,
    )

    # ── 5. Function Approximation ─────────────────────────────────
    d.page()
    d.hdr("Deep Learning as Function Approximator", S1)
    y = CT
    ...
    for i, ln in enumerate(
        ["× One row per state", "× No generalisation", "× Breaks at scale"]
    ):
    ...
    d.tx(
        rx + hw // 2, y - 18, "Q-Network  Q(s,a; θ)", bold=True, sz=13, cl=GREEN, a="c"
    )
    for i, ln in enumerate(
        ["✓ Generalises across states", "✓ Handles images/sensors", "✓ Scalable"]
    ):
    ...
    # ── 6. MDP / RL Framework ────────────────────────────────────
    d.page()
    d.hdr("The RL Framework: Markov Decision Process", S2)
    ...
    d.tx(
        ex + ew // 2, ey + eh // 2 + 4, "Environment", bold=True, sz=16, cl=GREEN, a="c"
    )
    ...
    # ── 7. Return & Discount ─────────────────────────────────────
    d.page()
    d.hdr("Return & the Discount Factor γ", S2)
    y = CT
    ...
    # ── 8. Value Functions ────────────────────────────────────────
    d.page()
    d.hdr("Value Functions: V(s) and Q(s,a)", S2)
    y = CT
    ...
    d.tx(
        rx8 + 14,
        y - 152,
        "DQN approximates Q*(s,a) directly!",
        bold=True,
        sz=10,
        cl=GREEN,
    )
    ...
    d.tx(
        ML + 14,
        y - 12,
        "Optimal policy:  π*(s) = argmaxₐ Q*(s,a)  — just pick the highest Q-value",
        sz=12,
    )

    # ── 9. Bellman Equation ───────────────────────────────────────
    d.page()
    d.hdr("The Bellman Optimality Equation", S2)
    y = CT
    d.tx(
        ML,
        y,
        "The optimal Q-function satisfies a recursive self-consistency equation:",
        sz=13,
    )
    y -= 30
    d.eqn(
    ...
    y -= 20
    y = d.bul(
        ML, y, "Q*(s,a) is defined in terms of itself — a recursive equation", sz=12
    )
    y = d.bul(
        ML,
        y,
        "Solved iteratively by repeated application (Q-learning, Value Iteration)",
        sz=12,
    )
    ...
    d.tx(
        ML + 14,
        y - 14,
        "→  Q-Learning solves this iteratively with tabular updates.",
        sz=12,
    )
    d.tx(
        ML + 14,
        y - 32,
        "→  DQN approximates Q* with a neural network trained by gradient descent.",
        sz=12,
    )
    ...
    # ── 10. TD Learning ──────────────────────────────────────────
    d.page()
    d.hdr("Temporal Difference (TD) Learning", S3)
    y = CT
    d.tx(
        ML,
        y,
        "TD learning updates value estimates online — no episode end needed.",
        sz=13,
    )
    y -= 30
    d.eqn(
        ...
        W - 2 * ML,
        60,
        ["V(sₜ) ← V(sₜ) + α · δₜ", "where  δₜ = rₜ + γ V(sₜ₊₁) − V(sₜ)   (TD error)"],
    ...
    for i, ln in enumerate(
        ["Wait until episode ends", "Use full return Gₜ", "High variance, unbiased"]
    ):
    ...
    for i, (ln, bold) in enumerate(
        ...
        d.tx(
            rx10 + 14, y - 50 - i * 26, ln, bold=bold, sz=11, cl=GREEN if bold else BODY
        )

    # ── 11. Q-Learning ────────────────────────────────────────────
    d.page()
    d.hdr("Q-Learning: Off-Policy TD Control", S3)
    y = CT
    d.tx(
        ML, y, "Q-learning applies TD directly to learn Q*(s,a) without a model.", sz=13
    )
    y -= 30
    d.eqn(
        ML,
    ...
    y -= 20
    y = d.bul(
        ML, y, "Off-policy: learns Q* regardless of which policy collects data", sz=12
    )
    y = d.bul(ML, y, "Model-free: doesn't need to know P(s'|s,a) or R(s,a)", sz=12)
    y = d.bul(
        ML,
        y,
        "Provably converges to Q* given sufficient exploration + decaying α",
        sz=12,
    )
    y -= 16
    ...
    d.tx(
        ML + 14,
        y - 12,
        "🔑  Q-Learning is the direct predecessor of DQN.",
        bold=True,
        sz=12,
    )
    d.tx(
        ML + 14,
        y - 28,
        "    DQN replaces the Q-table with a deep neural network.",
        sz=11,
        cl=GRAY,
    )
    ...
    d.page()
    d.hdr("Why Q-Learning Fails at Scale", S3)
    y = CT
    ...
    d.tx(
        ML + 14,
        y - 10,
        "✓  DQN solves Problems 1 & 2 with Experience Replay + Target Network.",
        sz=12,
        cl=GREEN,
    )
    ...
    # ── 13. DQN: The Big Idea ─────────────────────────────────────
    d.page()
    d.hdr("DQN: The Big Idea", S4)
    y = CT
    d.tx(
        ML,
        y,
        "DQN = Q-Learning  +  Deep Neural Network  +  two stabilising tricks.",
        sz=13,
    )
    y -= 28
    ...
    d.page()
    d.hdr("The Atari Breakthrough  (Mnih et al., Nature 2015)", S4)
    y = CT
    ...
    d.tx(
        ML + 14,
        y - 16,
        "🏆  One algorithm, raw pixels, superhuman performance on diverse games.",
        sz=12,
        cl=GREEN,
    )
    d.tx(
        ML + 14,
        y - 32,
        "    Demonstrated that deep RL can generalise end-to-end across tasks.",
        sz=11,
        cl=GRAY,
    )
    ...
    d.page()
    d.hdr("DQN Network Architecture", S5)
    ...
    d.page()
    d.hdr("Input Preprocessing (Atari)", S5)
    y = CT
    ...
    steps16 = [
        ...
        (
            "2. Crop & Resize",
            "Resize to 84×84 pixels  (standardise spatial resolution)",
        ),
        (
            "3. Frame Stack",
            "Stack 4 consecutive frames → 84×84×4  (captures motion & velocity)",
        ),
    ...
    d.tx(
        ML + 14,
        y - 12,
        "4 stacked frames give the network a sense of velocity and direction.",
        sz=12,
    )
    d.tx(
        ML + 14,
    ...
    d.page()
    d.hdr("The Two Core Problems DQN Solves", S6)
    y = CT
    d.tx(
        ML, y, "Adding a NN to Q-learning introduces two new training problems:", sz=13
    )
    y -= 26
    ...
    for i, ln in enumerate(
        [
            "Consecutive (s,a,r,s') are correlated.",
            "NNs need i.i.d. data.",
            "→ training diverges",
            "",
    ...
    d.tx(
        ML + 14,
        y - 10,
        "✓  DQN solves both with Experience Replay and a frozen Target Network.",
        sz=12,
        cl=GREEN,
    )
    ...
    d.page()
    d.hdr("Component 1: Experience Replay", S6)
    y = CT
    d.tx(
        ML, y, "Store all transitions in a buffer; train on random mini-batches.", sz=13
    )
    y -= 24
    ...
    d.tx(
        ML + 10,
        y - 8,
        "Mini-batch sampled randomly (highlighted above)",
        italic=True,
        sz=10,
        cl=ORANGE,
    )
    ...
    y -= 30
    for ln in [
        "Breaks temporal correlations — training data is i.i.d.",
    ...
    # ── 19. Target Network ────────────────────────────────────────
    d.page()
    d.hdr("Component 2: Target Network", S6)
    y = CT
    d.tx(
        ML, y, "Keep a frozen copy of the network to compute stable TD targets.", sz=13
    )
    y -= 26
    ...
    # ── 20. Epsilon-Greedy ────────────────────────────────────────
    d.page()
    d.hdr("Component 3: ε-Greedy Exploration", S6)
    y = CT
    ...
    y = BAR_Y - 20
    y = d.bul(
        ML, y, "High ε early: agent explores freely, populates the replay buffer", sz=12
    )
    ...
    # ── 21. Loss Function ─────────────────────────────────────────
    d.page()
    d.hdr("Component 4: The Loss Function", S6)
    y = CT
    ...
    y -= 20
    y = d.bul(
        ML, y, "Huber loss instead of MSE — more robust to large TD errors", sz=12
    )
    y = d.bul(
        ML, y, "Clip rewards to [−1, +1] to normalise across different games", sz=12
    )
    y = d.bul(
        ML, y, "Update online θ every step;  copy to θ⁻ every C ≈ 1000 steps", sz=12
    )
    y -= 14
    d.rb(ML, y - 34, W - 2 * ML, 40, LYEL, sk=GOLD, r=5)
    d.tx(
        ML + 14,
        y - 10,
        "⚠  Never backpropagate through the target yᵢ — it must be treated as a constant.",
        sz=12,
    )
    ...
    # ── 22. Algorithm ─────────────────────────────────────────────
    d.page()
    d.hdr("DQN Training Algorithm", S7)

    pseudo = [
        (
            "Initialise",
            "replay buffer D (capacity N),  online network Q(θ),  target θ⁻ ← θ",
        ),
        ("", ""),
        ("For episode", "1 … M:"),
    ...
    # ── 23. Data Flow ─────────────────────────────────────────────
    d.page()
    d.hdr("Data Flow: One Training Step", S7)
    ...
    d.page()
    d.hdr("Putting It All Together", S7)
    y = CT
    ...
    cv.circle(CCX + 18, CCY - 20, 8, fill=1, stroke=0)
    pole_l = 88
    pole_a = math.radians(10)
    px25 = CCX + pole_l * math.sin(pole_a)
    py25 = CCY - 20 + pole_l * math.cos(pole_a)
    cv.setStrokeColor(ORANGE)
    cv.setLineWidth(5)
    cv.line(CCX, CCY - 20, px25, py25)
    cv.setFillColor(ORANGE)
    cv.circle(px25, py25, 7, fill=1, stroke=0)
    ...
    # ── 26. DQN on CartPole ───────────────────────────────────────
    d.page()
    d.hdr("DQN Applied to CartPole: Step by Step", S8)
    y = CT
    ...
    # ── 27. Double DQN ────────────────────────────────────────────
    d.page()
    d.hdr("Double DQN (DDQN)", S9)
    y = CT
    d.tx(
        ML,
        y,
        "DQN overestimates Q-values — the same network selects and evaluates.",
        sz=13,
    )
    y -= 22
    ...
    d.tx(
        ML + 14,
        y - 34,
        "Problem: maximisation bias → systematic overestimation of Q-values",
        sz=10,
        cl=RED,
    )
    ...
    d.tx(
        ML + 14,
        y - 34,
        "Online θ selects,  target θ⁻ evaluates  →  decoupled, less biased",
        sz=10,
        cl=GREEN,
    )
    ...
    # ── 28. Dueling DQN ───────────────────────────────────────────
    d.page()
    d.hdr("Dueling Network Architecture", S9)
    y = CT
    ...
    d.tx(CMX + CMW // 2, CMY + CMH // 2, "Q = V + A − mean(A)", sz=10, cl=ORANGE, a="c")
    ...
    d.tx(
        ML + 14,
        y2 - 14,
        "Benefit: network learns V(s) even for actions that don't matter → faster convergence.",
        sz=12,
    )

    # ── 29. PER + Noisy + Rainbow ─────────────────────────────────
    d.page()
    d.hdr("Prioritised Replay · Noisy Nets · Rainbow", S9)
    y = CT
