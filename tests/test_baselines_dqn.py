...
class TestDQNInit:
    ...
    def test_missing_observation_encoder_raises(self, one_predator_one_prey, dqn_config):
        from multi_agent_package.core.gridworld import GridWorldEnv
        from multi_agent_package.actions.discrete_actions import DiscreteActionSpace
        from baselines.DQN.dqn import DQN

        env = GridWorldEnv(agents=one_predator_one_prey, size=5, perc_num_obstacle=0,
...
class TestDQNPersistence:
    ...
    def test_loaded_model_matches_original_greedy_actions(self, dqn_env, dqn_config, tmp_path):
        from baselines.DQN.dqn import DQN
        dqn_config["epsilon"] = 0.0
        algo = DQN(dqn_env, dqn_config)
        algo.train()
