...
def py(y_in):
    return PH - y_in * inch  # top → rl bottom
...
def box(l, t, w, h, fill=None, stroke=None, sw=1):
    ...
    if stroke:
        ...
        c.setLineWidth(sw)
    c.rect(
        px(l), py(t + h), pw(w), ph(h), fill=1 if fill else 0, stroke=1 if stroke else 0
    )
...
def txt(text, l, t, size=14, color=DGRAY, bold=False, align="left", font=None):
...
def slide_header(title, hcolor=NAVY, tcolor=WHITE, hh=0.82):
    ...
    wrapped_txt(title, 0.35, 0.13, 12.6, hh - 0.15, size=22, color=tcolor, bold=True)
...
def section_slide(title, subtitle=""):
    ...
    wrapped_txt(
        title, 0.5, 1.85, 12.33, 1.4, size=36, color=WHITE, bold=True, align="center"
    )
    if subtitle:
        wrapped_txt(
            subtitle,
...
def bullet_row(items, l, t, w, size=15, gap=0.38, color=DGRAY):
    ...
    for item in items:
        ...
        wrapped_txt(sym + s, l + pad, y_cur, w - pad, gap * 1.5, size=sz, color=color)
        y_cur += gap
    return y_cur


def simple_table(
    data, l, t, w, h, col_fracs=None, hfill=NAVY, htxt=WHITE, fs=12, alt=True
):
...
# ══════════════════════════════════════════════════════════════════════════════
section_slide("Section 1: Why This Exists", "The problem we solve and our mission")

# ── Slide 2 — The Problem ────────────────────────────────────────────────────
new_slide("The Problem with Most MARL Environments")
wrapped_txt(
    "Most research environments tangle concerns that should stay independent:",
    0.5,
    0.95,
    12.33,
    0.42,
...
for i, (col, title, sub, note) in enumerate(layers):
    ...
    wrapped_txt(
        title, 0.68, y_start + i * lh + 0.1, 8.6, 0.48, size=16, color=WHITE, bold=True
    )
    wrapped_txt(
        sub, 0.68, y_start + i * lh + 0.6, 8.6, 0.38, size=12, color=HexColor("#DDDDDD")
    )
    # right note box
    lighter = HexColor(
        "#{:02X}{:02X}{:02X}".format(
    ...
    wrapped_txt(note, 9.22, y_start + i * lh + 0.22, 3.4, 0.75, size=12, color=WHITE)

# ── Slide 5 — Design Invariants ──────────────────────────────────────────────
new_slide("Five Core Design Invariants")
wrapped_txt(
    "These rules protect the scientific validity of every experiment run on this codebase.",
    0.5,
    0.92,
    12.33,
    0.4,
...
wrapped_txt(
    "Plugins are discovered at runtime from YAML keys — no hardcoded imports in core.",
    0.5,
    0.92,
    12.33,
    0.4,
...
simple_table(
    ...
    12.5,
    5.6,
    col_fracs=[0.12, 0.27, 0.19, 0.22, 0.20],
    fs=13,
)
wrapped_txt(
    "All three pathways follow the identical discover-by-string-key pattern. "
...
# SECTION — Workflows
# ══════════════════════════════════════════════════════════════════════════════
section_slide("Section 3: Workflows", "Init, step-loop, and training loop in detail")
...
for i, (col, title, body) in enumerate(init_steps):
    ...
    wrapped_txt(
        title, xl + 0.12, yt + 0.1, bw - 0.24, 0.42, size=14, color=WHITE, bold=True
    )
    wrapped_txt(
        body,
        xl + 0.12,
...
wrapped_txt(
    "Result: a fully wired environment — observations, rewards, and algorithm connected without the algorithm knowing how.",
    0.65,
    5.6,
    12.0,
    0.5,
...
wrapped_txt(
    "⚠  Returns a dict — not the standard Gymnasium (obs, rew, term, trunc, info) tuple. "
...
wrapped_txt(
    "One directory = one responsibility.", 0.5, 0.92, 12.33, 0.4, size=13, color=MGRAY
)
code_block(
    """\
Predator-Prey-Archetype-Gridworld-Environment/
...
wrapped_txt(
    "GridWorldEnv(gym.Env)", 0.62, 1.1, 5.7, 0.48, size=16, color=RED, bold=True
)
...
for i, a in enumerate(
    ...
    wrapped_txt(
        a, 0.72, 2.05 + i * 0.34, 5.5, 0.32, size=11, color=DGRAY, font="Courier"
    )
...
for i, a in enumerate(
    ...
    wrapped_txt(
        a, 7.32, 2.05 + i * 0.36, 5.5, 0.32, size=11, color=DGRAY, font="Courier"
    )
...
wrapped_txt(
    "The rule: any algorithm written here must work with any env that implements reset() + step(). "
...
# ══════════════════════════════════════════════════════════════════════════════
section_slide(
    "Section 6: Connection to RL Theory", "From Bellman equations to running code"
)

# ── Slide 16 — MDP Mapping ───────────────────────────────────────────────────
new_slide("Single-Agent MDP — Every Component Maps to Code")
wrapped_txt(
    "A Markov Decision Process is (S, A, R, P, γ). "
...
wrapped_txt(
    "In this env: predators and prey are adversarial teams. "
...
wrapped_txt(
    "Each algorithm trades coordination ability against scalability.",
    0.5,
    0.92,
    12.33,
    0.4,
...
for col, title, desc, pros in algos:
    ...
    wrapped_txt(
        title, xl + 0.1, 2.05, 2.9, 0.5, size=13, color=WHITE, bold=True, align="center"
    )
...
wrapped_txt(
    "Adding CQL, MixedTrainer, and Nash Q each required: one file, one registry line, one YAML. Zero core changes.",
    0.5,
    0.92,
    12.33,
    0.4,
...
box(0.4, 6.82, 12.5, 0.55, fill=NAVY)
wrapped_txt(
    "Invariant: adding an algorithm never changes the environment, plugins, or other algorithms. "
...
simple_table(
    ...
    8.4,
    4.3,
    col_fracs=[0.12, 0.15, 0.25, 0.28, 0.20],
    fs=14,
)
...
wrapped_txt("dⁿ", 9.3, 2.05, 3.5, 0.85, size=52, color=RED, bold=True, align="center")
wrapped_txt(
...
wrapped_txt(
    "Every contribution fits one of these patterns. Following the pathway guarantees a clean merge.",
    0.5,
    0.92,
    12.33,
    0.4,
...
wrapped_txt(
    "What happens when you hit Enter:", 0.5, 0.92, 12.33, 0.4, size=14, color=MGRAY
)
...
for i, (col, step, desc) in enumerate(e2e):
    ...
    wrapped_txt(
        desc, xl + 0.08, 2.87, bw2 - 0.24, 0.92, size=11, color=DGRAY, align="center"
    )
...
wrapped_txt(
    "How to read the results", 6.62, 1.56, 6.3, 0.45, size=14, color=NAVY, bold=True
)
insights = [
    (
        "Reward curve rising",
...
for s in [
    ...
    wrapped_txt("✓  " + s, 0.65, y_, 5.3, 0.38, size=14, color=DGRAY)
    y_ += 0.52
...
for week, item in roadmap:
    ...
    wrapped_txt(
        week, 6.97, y_ + 0.1, 1.2, 0.38, size=12, color=NAVY, bold=True, font="Courier"
    )
...
for col, title, items in cards:
    ...
    for item in items:
        wrapped_txt(
            "•  " + item, xl + 0.15, y_, 3.7, 0.65, size=12, color=HexColor("#CCE0EE")
        )
        y_ += 0.78
    xl += 4.4
