...
def txt(sl, text, l, t, w, h, size=16, bold=False, italic=False,
...
def mltext(sl, lines, l, t, w, h, def_size=15, def_color=None, font="Calibri"):
    ...
    for item in lines:
        ...
        first = False
        r = p.add_run()
        r.text = s; r.font.size = Pt(sz); r.font.bold = b
...
def code(sl, src, l, t, w, h, size=11):
    ...
    for line in src.split("\n"):
        ...
        r.font.name = "Courier New"; r.font.color.rgb = DGRAY
...
def tbl(sl, data, l, t, w, h, hfill=NAVY, htxt=WHITE,
        col_w=None, fs=13, row_fills=None):
    rows, cols = len(data), len(data[0])
    t_ = sl.shapes.add_table(
        rows, cols, Inches(l), Inches(t), Inches(w), Inches(h)).table
    if col_w:
        emu = Inches(w)
    ...
    for ri, row in enumerate(data):
        for ci, val in enumerate(row):
            ...
            f = cell.fill
            if ri == 0:
                f.solid(); f.fore_color.rgb = hfill
            elif row_fills and ri-1 < len(row_fills) and row_fills[ri-1]:
                f.solid(); f.fore_color.rgb = row_fills[ri-1]
...
def section(title, sub=""):
    ...
    txt(sl, title, 0.5, 1.9, 12.33, 1.3,
        size=38, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
    if sub:
        txt(sl, sub, 0.5, 3.55, 12.33, 0.8,
            size=20, color=RGBColor(0xB0, 0xC4, 0xDE), align=PP_ALIGN.CENTER)
    return sl

def cslide(title, hc=NAVY):
    sl = S(); bg(sl, WHITE); hdr(sl, title, hc=hc); return sl
...
txt(sl, "Predator-Prey Gridworld", 0.6, 1.1, 12.13, 1.4,
    size=44, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
txt(sl, "A Modular Multi-Agent Reinforcement Learning Testbed",
    0.6, 2.75, 12.13, 0.8,
    size=24, color=RGBColor(0xB0, 0xC4, 0xDE), align=PP_ALIGN.CENTER)
rect(sl, 1.0, 3.7, 11.33, 0.05, fill=ORANGE)
txt(sl, "Student Orientation  •  Week 1", 0.6, 3.95, 12.13, 0.5,
    size=18, color=RGBColor(0x90, 0xA8, 0xC8), align=PP_ALIGN.CENTER)
txt(sl, "Predator-Prey Archetype Gridworld Environment",
    0.6, 4.55, 12.13, 0.5,
...
txt(sl, "Most research environments tangle concerns that should be independent:",
    0.5, 1.0, 12.33, 0.45, size=16, color=DGRAY)
bullets(sl, [
...
], 0.5, 1.55, 12.33, 3.2, size=17)
rect(sl, 0.5, 5.0, 12.33, 1.6, fill=RGBColor(0xE8, 0xF4, 0xFD), border=BLUE)
txt(sl, '"We need an environment where each layer can be studied independently,\n'
        'with full reproducibility and zero surprise interactions between components."',
    0.8, 5.15, 11.8, 1.3, size=16, italic=True, color=NAVY)
...
y = 1.65
for s in ["A controlled lab for teaching and researching RL/MARL",
    ...
    txt(sl, "✓  " + s, 0.65, y, 5.3, 0.55, size=14, color=DGRAY); y += 0.7
...
y = 1.65
for s in ["A high-performance training platform",
    ...
    txt(sl, "✗  " + s, 7.15, y, 5.3, 0.55, size=14, color=DGRAY); y += 0.7

txt(sl, "← designed for this", 3.5, 6.6, 3.0, 0.4, size=13, color=GREEN, align=PP_ALIGN.RIGHT)
...
txt(sl, "Every component lives in exactly one layer. Dependencies flow downward only.",
    0.5, 0.92, 12.33, 0.42, size=14, color=MGRAY)
layers = [
...
for col, title, sub, note in layers:
    ...
    lighter = RGBColor(min(255, col[0]+55), min(255, col[1]+55), min(255, col[2]+55))
...
txt(sl, "These rules protect the scientific validity of every experiment.",
    0.5, 0.92, 12.33, 0.42, size=14, color=MGRAY)
invariants = [
    ("Immutable Core",     "src/core/ is never modified by contributors. Physics must not change between experiments."),
    ("Seeded Randomness",  "env.reset(seed=n) with the same n always produces the exact same trajectory."),
    ("Read-Only Plugins",  "Observation builders and reward functions observe state — they never write it."),
    ("Config Drives All",  "If you need behavioral change, write a plugin. If you think you need to edit core, that is a design bug."),
    ("Black-Box Algos",    "Algorithms may only call env.reset() and env.step(). No peeking at env.agents or internal state."),
]
y = 1.45
...
txt(sl, "Plugins are discovered at runtime from YAML keys — no hardcoded imports in core code.",
    0.5, 0.92, 12.33, 0.42, size=14, color=MGRAY)
boxes = [
...
for x, title, body, col in boxes:
    ...
    txt(sl, title, x+0.1, 1.6, 2.0, 0.42, size=13, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
    txt(sl, body,  x+0.1, 2.06, 2.0, 1.0, size=11, color=WHITE, align=PP_ALIGN.CENTER, font="Courier New")
for xi in [2.62, 5.22, 7.82, 10.42]:
    txt(sl, "→", xi, 2.1, 0.4, 0.4, size=22, bold=True, color=NAVY, align=PP_ALIGN.CENTER)
...
tbl(sl, [
    ...
    ["Algorithm",   "BaseAlgorithm.select_actions()\n.train()", "src/baselines/", "algorithm_registry.py",  "experiment.algorithm.name"],
], 0.4, 1.1, 12.5, 5.6,
   hfill=NAVY, col_w=[0.13, 0.28, 0.20, 0.22, 0.17], fs=14)
txt(sl, "All three follow the identical discover-by-string-key pattern. New contributors only ever touch one pathway.",
    0.5, 6.85, 12.33, 0.42, size=13, italic=True, color=MGRAY)
...
txt(sl, "What happens between  python run_from_config.py  and the first  env.step() ?",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
init_steps = [
...
for i, (col, title, body) in enumerate(init_steps):
    ...
    txt(sl, body,  x+0.12, y+0.55, bw-0.24, 0.78, size=12, color=WHITE, font="Courier New")

rect(sl, 0.4, 5.5, 12.5, 0.62, fill=NAVY)
txt(sl, "Result: a fully wired environment — observations, rewards, and algorithm all connected without the algorithm knowing how.",
    0.65, 5.58, 12.0, 0.48, size=14, bold=True, color=WHITE, align=PP_ALIGN.CENTER)

# Slide 9 — Per-Step Pipeline
sl = cslide("Per-Step Pipeline — Inside  env.step(actions)")
txt(sl, "Called once per timestep. Every stage is deterministic given the same state and seed.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
steps9 = [
...
txt(sl, "⚠  Returns a dict — not the standard Gymnasium (obs, rew, term, trunc, info) tuple. Gymnasium wrapper is a Week-1 deliverable.",
    0.65, y+0.1, 12.0, 0.38, size=13, color=AMBER)
...
txt(sl, "One directory = one responsibility. If something is hard to place, that is a design signal.",
    0.5, 0.92, 12.33, 0.42, size=14, color=MGRAY)
code(sl, """\
...
for q, a in guide:
    ...
    txt(sl, "→  " + a, 9.5, y+0.28, 3.4, 0.3, size=12, bold=True, color=TEAL, font="Courier New")
    y += 0.66
...
tbl(sl, [
    ...
    ["registry/observation_registry.py", "L2 Reg", "Map YAML string keys to builder classes"],
   ...
   hfill=NAVY, col_w=[0.37, 0.17, 0.46], fs=13)


# ══════════════════════════════════════════════════════════════════════════════
# SECTION 5 — Class & Code Structure
# ══════════════════════════════════════════════════════════════════════════════
section("Section 5: Class & Code Structure", "The key abstractions and how they connect")
...
for i, a in enumerate(["agents              list[Agent]",
...
for i, m in enumerate(["reset(seed)  →  obs_dict",
...
for i, a in enumerate(["agent_name     str",
...
txt(sl, "0=Right  1=Up  2=Left  3=Down  4=Noop",
    7.3, 4.95, 5.5, 0.32, size=12, color=NAVY, font="Courier New")
txt(sl, "All agents move simultaneously — not turn-by-turn.",
    7.3, 5.4, 5.5, 0.38, size=12, italic=True, color=MGRAY)
...
txt(sl, "What the algorithm sees — and what it must never access.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
...
for a in ["env.reset(seed=n)",
          ...
          "env.action_space"]:
    txt(sl, "✓  " + a, 0.7, y, 5.4, 0.36, size=13, color=GREEN, font="Courier New"); y += 0.42
...
for a in ["env.agents",
          ...
          "env.reward_fn",
          "env.observation_builder"]:
    txt(sl, "✗  " + a, 7.2, y, 5.5, 0.36, size=13, color=RED, font="Courier New"); y += 0.52

rect(sl, 0.4, 6.82, 12.5, 0.55, fill=NAVY)
txt(sl, "The rule: any algorithm written here must work with any env that implements reset() + step(). That is what makes baselines reusable across all experimental conditions.",
...
txt(sl, "A Markov Decision Process is (S, A, R, P, γ). Here is exactly where each lives in this project.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
tbl(sl, [
...
], 0.4, 1.5, 12.5, 5.2, hfill=NAVY, col_w=[0.17, 0.09, 0.44, 0.30], fs=13)

# Slide 17 — Bellman Equation
sl = cslide("The Bellman Equation — Math to Code")
txt(sl, "The update rule that drives all tabular Q-learning in this project.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
rect(sl, 0.5, 1.42, 12.33, 1.05, fill=LGRAY, border=RGBColor(0xBB,0xBB,0xBB))
txt(sl, "Q(s, a)  ←  Q(s, a)  +  α  ·  [ r  +  γ · max  Q(s', a')  −  Q(s, a) ]",
    0.7, 1.52, 12.0, 0.82, size=26, bold=True, color=NAVY, align=PP_ALIGN.CENTER)
...
for col, label, desc, x in terms:
    ...
    txt(sl, label, x+0.1, y2+0.08, 2.0, 0.5, size=15, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
    txt(sl, desc, x+0.1, y2+0.65, 2.0, 1.3, size=12, color=WHITE, align=PP_ALIGN.CENTER)

code(sl, "td_error = reward + gamma * max(q_tables[agent][s_next].values()) - q_tables[agent][s][a]\nq_tables[agent][s][a] += alpha * td_error",
     0.5, 5.05, 12.33, 1.35, size=15)
txt(sl, "q_tables is a defaultdict(lambda: defaultdict(float)) — new states auto-init to zero.",
    0.7, 6.55, 12.0, 0.45, size=13, italic=True, color=MGRAY)

# Slide 18 — MDP → Markov Game
sl = cslide("From MDP to Markov Game — The MARL Jump")
txt(sl, "Adding more agents changes the problem structure — not just its size.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
rect(sl, 0.4, 1.45, 5.95, 4.9, fill=RGBColor(0xF5,0xF8,0xFF), border=BLUE)
txt(sl, "Single-Agent MDP", 0.6, 1.5, 5.5, 0.48, size=17, bold=True, color=BLUE, align=PP_ALIGN.CENTER)
y = 2.1
for s in ["State:   S",
          "Action:  A  (one agent)",
          "Reward:  R(s, a)",
    ...
    txt(sl, s, 0.65, y, 5.5, 0.36, size=14, color=DGRAY, font="Courier New"); y += 0.42

rect(sl, 6.9, 1.45, 5.95, 4.9, fill=RGBColor(0xFF, 0xF5, 0xF0), border=ORANGE)
txt(sl, "Markov Game  (MARL)", 7.1, 1.5, 5.5, 0.48, size=17, bold=True, color=ORANGE, align=PP_ALIGN.CENTER)
y = 2.1
for s in ["State:   S  (shared by all agents)",
    ...
    txt(sl, s, 7.15, y, 5.6, 0.36, size=13, color=DGRAY, font="Courier New"); y += 0.4

rect(sl, 0.4, 6.45, 12.5, 0.85, fill=NAVY)
txt(sl, "In this env: predators and prey are adversarial teams. Each team's reward depends on the other's actions. Neither side is stationary from the other's perspective.",
...
# ══════════════════════════════════════════════════════════════════════════════
section("Section 7: The MARL Algorithm Spectrum", "How the architecture enables different coordination strategies")

# Slide 19 — Spectrum
sl = cslide("The Algorithm Spectrum — Decentralised to Centralised")
txt(sl, "Each algorithm makes a different trade-off between coordination ability and scalability.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
# Spectrum bar
for i, c in enumerate([BLUE, TEAL, ORANGE, RED]):
    rect(sl, 0.4 + i*3.23, 1.52, 3.23, 0.4, fill=c)
txt(sl, "Decentralised ←──────────────────────────────────────────────────── Centralised",
    0.42, 1.54, 12.46, 0.36, size=13, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
...
for col, title, desc, pros in algos:
    ...
    txt(sl, title, x+0.1, 2.08, 2.9, 0.5, size=13, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
...
txt(sl, "Adding CQL and MixedTrainer required: one file each, one registry line each, one YAML each. Zero core changes.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
examples = [
...
for col, title, before, after in examples:
    ...
    txt(sl, "→", 6.2, y+0.85, 0.5, 0.4, size=22, bold=True, color=col, align=PP_ALIGN.CENTER)
...
rect(sl, 0.4, 6.82, 12.5, 0.55, fill=NAVY)
txt(sl, "The invariant: adding an algorithm never changes the environment, plugins, or other algorithms. Scientific comparability is always preserved.",
    0.65, 6.88, 12.0, 0.44, size=12, color=WHITE)

# Slide 21 — Scaling Wall
sl = cslide("The Scaling Wall — When CQL Breaks Down")
txt(sl, "Joint action spaces grow exponentially. This is why deep MARL methods exist.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
tbl(sl, [
...
txt(sl, "One rule protects the scientific integrity of every experiment run on this codebase.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
rect(sl, 0.9, 1.42, 11.53, 1.45, fill=NAVY, border=ORANGE)
txt(sl, "core/gridworld.py  and  core/agent.py  are NEVER modified by contributors.",
    1.1, 1.5, 11.1, 0.52, size=22, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
txt(sl, "All variation is expressed through config files, observation plugins, reward plugins, and algorithm plugins.",
    1.1, 2.12, 11.1, 0.65, size=16, color=AMBER, align=PP_ALIGN.CENTER)
...
], 0.5, 3.72, 12.33, 3.0, size=16)

# Slide 23 — Three Pathways
sl = cslide("The Three Contribution Pathways")
txt(sl, "Every contribution fits one of these patterns. Following the pathway = guaranteed clean merge.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
pathways = [
    (GREEN,  "Pathway 1: New Observation Builder",
    ...
    (ORANGE, "Pathway 2: New Reward Function",
    ...
    (PURPLE, "Pathway 3: New Learning Algorithm",
...
# ══════════════════════════════════════════════════════════════════════════════
section("Section 9: End-to-End Example", "One command, traceable from YAML to trained policy")

# Slide 24 — The Scenario
sl = cslide("The Scenario — Train Predators to Capture Prey")
txt(sl, "Goal: 2 predators learn to capture 2 prey on a 6×6 grid using IQL + distance shaping.",
    0.5, 0.92, 12.33, 0.42, size=15, italic=True, color=MGRAY)

code(sl, "# env.yaml\ngrid_size: 6\nmax_steps: 200\ncapture_threshold: 1\nperc_num_obstacle: 15.0\nrender_mode: null\nseed: 42",
     0.4, 1.4, 3.8, 2.4, size=12)
code(sl, "# agents.yaml\nagents:\n  - name: pred_0\n    type: predator\n    team: predators\n  - name: pred_1\n    type: predator\n    team: predators\n  - name: prey_0\n    type: prey\n    team: prey\n  - name: prey_1\n    type: prey\n    team: prey",
     4.4, 1.4, 3.8, 4.0, size=12)
code(sl, "# observations.yaml\nobservation:\n  type: local_radius\n  params:\n    radius: 3\n\n# rewards.yaml\nrewards:\n  - name: base_reward\n    weight: 1.0\n  - name: predator_distance\n    weight: 0.5\n\n# experiment.yaml\nexperiment:\n  algorithm:\n    name: iql\n  episodes: 1000\n  alpha: 0.1\n  gamma: 0.95\n  epsilon: 1.0\n  epsilon_decay: 0.995\n  min_epsilon: 0.01",
     8.4, 1.4, 4.55, 5.4, size=11)

code(sl, "# ONE COMMAND — no training code written\npython -m multi_agent_package.scripts.run_from_config --config configs/",
     0.4, 3.95, 7.8, 0.9, size=14)
txt(sl, "The only code you write is config. The system assembles everything.",
    0.6, 5.05, 7.6, 0.42, size=13, italic=True, color=MGRAY)

# Slide 25 — From Config to Training
sl = cslide("One Command — Full Training Run")
txt(sl, "What happens when you hit Enter:",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
rect(sl, 0.4, 1.38, 12.5, 0.72, fill=NAVY)
txt(sl, "python -m multi_agent_package.scripts.run_from_config  --config configs/",
    0.65, 1.45, 12.0, 0.58, size=18, bold=True, color=AMBER, font="Courier New")
...
for i, (col, step, desc) in enumerate(e2e):
    ...
    txt(sl, step, x+0.08, 2.34, bw2-0.24, 0.42, size=14, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
    rect(sl, x, 2.8, bw2-0.08, 1.05, fill=LGRAY, border=col)
    txt(sl, desc, x+0.08, 2.87, bw2-0.24, 0.92, size=12, color=DGRAY, align=PP_ALIGN.CENTER)
...
txt(sl, ("Episode  100/1000  |  avg_reward: -12.4  |  captures: 0.12  |  epsilon: 0.607\n"
    ...
    0.6, 4.55, 12.0, 2.45, size=13, color=RGBColor(0x00,0xFF,0x41), font="Courier New")

# Slide 26 — Observing the Result
sl = cslide("What Success Looks Like")
txt(sl, "Same seed + same YAMLs reproduces this result exactly on any machine.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)
tbl(sl, [
   ...
   hfill=TEAL, col_w=[0.48, 0.27, 0.25], fs=14)
code(sl, "python -m multi_agent_package.scripts.evaluate \\\n  --checkpoint checkpoints/ --episodes 200",
     0.4, 5.6, 5.7, 0.95, size=13)
...
txt(sl, "Ten weeks, four students, one JOSS paper.",
    0.5, 0.92, 12.33, 0.42, size=14, italic=True, color=MGRAY)

rect(sl, 0.4, 1.45, 5.85, 5.35, fill=RGBColor(0xF0,0xFB,0xF4), border=GREEN)
txt(sl, "Current State  (start of Week 1)", 0.6, 1.5, 5.5, 0.45, size=15, bold=True, color=GREEN)
y = 2.05
for s in ["Discrete gridworld (up to 10×10)",
    ...
    txt(sl, "✓  " + s, 0.65, y, 5.3, 0.38, size=14, color=DGRAY); y += 0.52
...
for week, item in roadmap:
    ...
    txt(sl, week, 6.95, y+0.1, 1.2, 0.38, size=12, bold=True, color=NAVY, font="Courier New")
...
sl = S(); bg(sl, NAVY)
rect(sl, 0, 0, 13.33, 0.32, fill=ORANGE)
txt(sl, "Resources & Next Steps", 0.5, 0.55, 12.33, 0.85,
...
for col, title, items in cards:
    ...
    y = 2.5
    for item in items:
        txt(sl, "•  " + item, x+0.15, y, 3.7, 0.62, size=12,
            color=RGBColor(0xCC,0xDD,0xEE)); y += 0.75
    x += 4.4

txt(sl, "Build understanding from the ground up.",
    0.5, 7.1, 12.33, 0.35, size=16, italic=True, color=AMBER, align=PP_ALIGN.CENTER)
