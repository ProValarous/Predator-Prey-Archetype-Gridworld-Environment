...
- **PR #22 — best of the three here.** `DQN._encode_observation` calls `env.observation_encoder` generically, so the *same* DQN class works with `default`, `local_only`, `local_radius`, `absolute`, and `relative` observation types purely by changing `plug-and-play/configs/observations.yaml`. `local_radius.encode()` pads to `len(env.agents)-1` / `len(env._obstacle_location)` automatically — no manual "max agents" tuning needed. `state_dim` and `action_dim` are both auto-inferred at construction time (`_infer_action_dim`, encoding the first reset observation) with a fail-fast check if `observation_encoder` is missing.
...
- PR #22's shipped `plug-and-play/configs/experiment_dqn.yaml` defaults to `episodes: 5` — looks like a leftover smoke-test value rather than a usable training default; #23 (200) and #24 (1000) ship sensible defaults.
